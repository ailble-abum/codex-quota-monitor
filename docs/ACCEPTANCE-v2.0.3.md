...
发布 `v2.0.3` 后，再从 GitHub 最新正式发行接口下载该版本的 ZIP 与 `SHA256SUMS`，在全新独立目录重复完整升级。合成页面先显示 `plugin 2.0.0`，真实 LaunchAgent bootout/bootstrap 后显示 `plugin 2.0.3`；只有一个面板，测试标签与临时目录已清理。此项复测核对了用户实际可下载的正式包，而不只是在发布前的候选文件。
