...
- `codex-quota-monitor-v2.0.0-beta.1-devkit.zip`：本版本源码、测试、交接记录及六份固定构建输入；适合另一台 Mac 离线重建 renderer。Python 依赖和浏览器测试运行时仍需单独安装。
...
内嵌面板需要宿主已经提供明确的本机回环调试端点。本包不会开启端口、启动或重启宿主。如果普通打开 Codex 后没有端点，本版只能安静等待，无法显示面板；不要把它当作安装损坏。没有端点的测试者可先做源码及合成测试。
...
先读 `AGENTS.md`、本文和 `docs/HANDOFF-2026-09-20.md`。开发工作基于 Beta 分支，不合并到稳定版 `main`。`devkit.zip` 内的源码是发布快照，不含 `.git`；可把其中的 `frozen-source` 复制到克隆目录，用于下列重建。此目录及私人配置已列入忽略规则。

没有开发包时，可从现有公共仓库取得同一提交的固定输入（不运行旧版脚本）：

```sh
git clone --no-checkout https://github.com/ailble-abum/codex-quota-monitor.git ../codex-quota-monitor-source
python3 - <<'PY'
from pathlib import Path
import subprocess
base = '2705f32a6f080ee1bdbecdc5b43f5fe0f9045eed'
target = Path('frozen-source')
target.mkdir()  # 已存在则停止，保留原材料。
names = ['LICENSE', 'NOTICE', 'context_token_injector.py',
         'companion_feedback.py', 'companion_art.py', 'companion_expressions.py']
for name in names:
    source = name if name in ('LICENSE', 'NOTICE') else 'plugins/codex-quota-monitor/scripts/' + name
    data = subprocess.check_output(['git', '-C', '../codex-quota-monitor-source', 'show', base + ':' + source])
    (target / name).write_bytes(data)
PY
```

在源码目录或解压后的开发包目录中重建；三个输出目录必须事先不存在：

```sh
python3 -m venv .venv
.venv/bin/python -m pip install -r requirements-cdp.txt
.venv/bin/python -m unittest discover -s tests
.venv/bin/python tools/build_panel_candidate.py frozen-source candidate
.venv/bin/python tools/install_preview.py candidate preview
.venv/bin/python preview/run.py --help
```

构建器核对六个输入的固定 SHA256；不接受未知旧版本。生成的 preview 继续保留来源声明，默认只是技术预览。运行包的生成基于这个入口，不以拷贝本机现用安装、虚拟环境、用户配置或会话数据制作包。
...
两个压缩包均只从受版本控制的源码和已核对的固定输入生成，附带 LICENSE/NOTICE。运行包文件摘要见 `install-manifest.json`；发行信息见 `BETA-RELEASE.json`。开发包同样记录发行信息。源码 Git 历史单独通过 Beta 分支保留。
