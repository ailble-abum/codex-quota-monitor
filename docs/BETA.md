...
当前功能仍有缺口：普通启动自动跟随及整应用重启恢复未通过；健康提示和显式配置的本地数值摘要已接入 V2 合成运行链，但旧版周报、菜单栏和系统配额通知尚未替换或接通。预览 renderer 已由 V2 模块独立构建，但伴宠视觉资源来源仍待完整复核，继续保留原 LICENSE/NOTICE，不宣称整份发行内容已完成来源审查。真实窗口刷新只有历史证据，本 Beta 包不据此宣称新一轮原生验收。
