...
2026-09-24。首个正式版限定 macOS；Windows 不在本轮发行范围。本文保存发行前的验收门槛与历次候选问题；最终结果见末节。旧版 `main` 的未提交工作保持原样。
...
任一步失败时保存退出码、`doctor` 的安全状态字段和复现操作即可；不要共享认证、完整会话 JSONL 或私有配置。

## 2026-09-24 最终验收结果

- 用户确认 V2 面板与菜单栏实际可见，正常退出并重新打开 Codex、重新登录 Mac 后两者均恢复。此前仅凭进程和状态标记作出的可见性判断未计为通过。
- 回退测试先卸载 V2 菜单栏再卸载主服务，两个 V2 标签均显示未安装；旧版服务、菜单栏、面板和 injector 恢复正常。随后再次停旧版、安装 V2，`doctor` 返回 `service=running`、`config=valid`、`panel=updated`，菜单栏服务为 `running`。
- Python 3.9 全套 198 项、Swift 类型检查、合成 Chromium/WebKit 测试通过。正式 ZIP 解压后文件审计与 CLI 检查通过；发布后从 GitHub 下载的 ZIP 与本地构建逐字节一致，`SHA256SUMS` 校验通过。
- [macOS v2.0.0 正式版](https://github.com/ailble-abum/codex-quota-monitor/releases/tag/v2.0.0) 已发布。默认分支切为 `v2`；旧版 `main` 保留，不覆盖其提交。Intel Mac、Windows、签名与公证不在本轮通过范围。
