...
| 值 | 证据与边界 |
| --- | --- |
| closed | 本实例没有尝试初始化消费者或成功发布快照，连接已关闭 |
| released | 页面已确认释放请求；另一个 owner 的状态会被保留 |
| lease_pending | 曾初始化消费者或发布快照，但已断连、取消发生在收发途中，或页面释放失败；无法确认即时清空，依赖已有 120 秒租期/消费者恢复 |
...
受管候选的可选卸载回调与 released/lease_pending 具体边界见[消费者卸载契约](consumer-dispose-contract.md)。
