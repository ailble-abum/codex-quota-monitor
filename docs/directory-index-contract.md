...
- 每轮所有文件合计最多读取 256 KiB，均分预算，保留增量游标。普通文件仍有 1 MiB 的未完成行上限；明确选择 `codex-rollout` 布局时，为兼容压缩记录使用 2 MiB 上限。128 个文件的待解析字节最坏约 128 MiB，另有解析临时对象。大目录应由调用方明确缩小授权范围；此阶段未做生产规模性能验收。
