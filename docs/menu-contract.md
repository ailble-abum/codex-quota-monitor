...
2026-09-25 Token 活动补充：复用现有官方 `account/usage/read` 投影和侧栏“最近日用量/累计活动”口径，只从 live 账户快照的最后一个日桶及累计摘要保存两个经过范围检查的数值。菜单只展示最近 120 秒的当前账户采样；缺字段显示 `—`，过期时不显示活动。历史文件不保存原始日桶、日期或账户响应，也不把样本数当作 Token 消耗。合成双账户、非 live、缺失/过期和数值白名单测试通过；真实菜单 UI 与账户读取需独立验收。
