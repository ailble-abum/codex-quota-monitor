...
- 页面通过 window.__quotaMonitorV2Thread 显式提供任务 ID；支持 local: 前缀。此字段是隔离集成协议，不能宣称真实 Codex 已提供它。
...
- getter 的过期保护只约束再次读取。消费者若复制快照或把数字画进 DOM，必须自行响应失效并清空；本轮没有新增 UI，也未完成旧面板消费桥的连接。
...
没有访问真实会话或改变现用安装；未完成原生 macOS Codex UI、Windows、安装包、旧面板更新适配及发行来源验收。下一步：依据来源已核实的宿主接口实现活动任务识别，将本轮数值桥接入保留的面板，并验证消费者失效清空；不能把合成 bridge 当作已替换旧 injector。
