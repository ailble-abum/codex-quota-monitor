...
class UpdateStateTests(unittest.TestCase):
    def test_release_version(self):
        self.assertEqual(CURRENT_VERSION, '2.0.7')

    def test_projection_is_bounded_and_compares_release_versions(self):
        self.assertEqual(project({'tag_name': 'v2.0.8', 'html_url': 'https://example.invalid/release'})['status'],
                         'update_available')
        ...
        self.assertEqual(project({'version': '2.0.8', 'url': 'http://example.invalid'})['status'],
                         'update_available')
