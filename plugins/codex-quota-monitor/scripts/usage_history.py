    ...
    def page(rows):
        ...
        if daily:
            ...
            if clean:
                ...
                bars=''.join(f'<div class="bar" tabindex="0" role="img" style="height:{max(2,100*float(item["tokens"])/peak):.1f}%" title="{html.escape(item["startDate"])} · {int(item["tokens"]):,} Token" aria-label="{html.escape(item["startDate"])} · {int(item["tokens"]):,} Token"></div>' for item in clean)
        ...
        return '<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Codex · 本地周报</title><style>:root{color-scheme:light dark}body{max-width:850px;margin:40px auto;padding:0 24px;background:color-mix(in srgb,CanvasText 4%,Canvas);color:CanvasText;font:14px/1.6 system-ui}section{background:Canvas;padding:24px;border-radius:18px;margin:20px 0}h1{font-size:24px}h2{font-size:15px}svg{width:100%}text{font:11px system-ui;fill:CanvasText;opacity:.62}p{color:color-mix(in srgb,CanvasText 62%,Canvas)}.cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px}.cards div{padding:14px;border-radius:12px;background:color-mix(in srgb,CanvasText 4%,Canvas)}.cards strong,.cards span{display:block}.cards strong{font-size:20px}.cards span{color:color-mix(in srgb,CanvasText 62%,Canvas)}.bars{height:150px;display:flex;gap:4px;align-items:end;border-bottom:1px solid color-mix(in srgb,CanvasText 14%,Canvas)}.bar{flex:1;min-width:3px;background:#285dc1;border-radius:3px 3px 0 0}.bar:focus-visible{outline:2px solid CanvasText;outline-offset:2px}.rank{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,3fr) auto;gap:10px;align-items:center;margin:10px 0}.rank span{min-width:0;overflow-wrap:anywhere}.rank b{display:block;height:9px;background:#285dc1;border-radius:9px}.rank strong{font-variant-numeric:tabular-nums}</style><h1>本地用量周报</h1><p>本地记录 · 最近 7 天 · 仅当前采集账户 · '+str(len(rows))+' 个采样点。配额重置与超过 3 分钟的空档分段绘制。只从启用后开始记录，不补造历史。</p>'+report+usage_summary+breakdown_html+''.join(charts)+''.join(metric_charts)+'<p>数据更新时间：'+time.strftime('%Y-%m-%d %H:%M:%S')+'。重新打开本页查看更新。关闭 Codex 时采集暂停。</p></html>'
