...
The whole panel is a grab surface: drag it from the title row, the quota readout, or any gap between rows. Only controls that own the pointer keep it -- buttons, the unit selector, the disclosure summaries, and the four invisible corner resize hit areas. The cursor is their only mouse cue, so resizing no longer consumes panel space; the southeast corner remains keyboard-focusable and arrow keys resize it in 12px steps. A pointerdown on the scrollbar gutter still belongs to the scroll container, so scrolling never turns into a drag. Compact and expanded positions/sizes are remembered separately. Temporary viewport clamping does not overwrite the saved anchor. A 64px top safe area avoids the app title bar, and the overlay is explicitly marked as a non-native-drag region. The title row is pinned to the top of the panel, so the quota readout and the window controls stay reachable while the detail list is scrolled.

Soft edge docking is enabled by default. Release the panel within 14px of the left or right wall to tuck it away behind a small companion; hovering reveals the unchanged panel toward the screen interior, leaving hides it after 500ms, and clicking the companion pins it open. Drag the docked companion vertically to save a new height; a 4px threshold keeps an ordinary click from becoming a move. Drag the revealed header to detach it. Only the side walls dock: every companion is drawn as a figure peeking in from a vertical edge, so a top or bottom dock would have to lay the character on its side. A layout saved by an earlier build that used those edges is migrated to a free-floating panel. A docked bitmap companion draws no card and no plate -- the character sits against the window edge with the account gauge beside it, and only the vector fallback still gets the glass card it needs for contrast on a light canvas. Display settings can disable docking and choose between the companions; the picker is a collapsible section whose summary names the live skin, so a collapsed picker still says which one is docked.

小伴侣大小默认自动适配窗口：以 1440×900 逻辑像素为基准，`clamp(1, min(窗口宽/1440, 窗口高/900), 1.5)`，图片高度为 48～72px。显示设置中的“小伴侣大小”滑块支持 75%～200%，立即生效并保存在本机；“恢复自动”清除手动值。角色、上下文圆环、额度条与点击区域同步缩放；位置与面板尺寸偏好分别保存。

Implementation choice (2026-09-20): reuse the existing resize listener, localStorage settings and native range input; use [CSS zoom](https://developer.mozilla.org/en-US/docs/Web/CSS/zoom) on the companion as a whole. Fixed top offsets are divided by the scale to preserve viewport coordinates. No new runtime dependencies. Window dimensions are CSS pixels, not physical screen inches or devicePixelRatio. Chromium and WebKit fixture tests cover auto/manual sizing, reload persistence, six skins on both walls, dragging, keyboard input and light/dark rendering; this is browser evidence, not a physical multi-monitor or Windows acceptance claim. With Playwright available, run `node scripts/test_companion_scale_ui.cjs` (optionally set `NODE_PATH`, `PYTHON`, and `ARTIFACT_DIR`).
