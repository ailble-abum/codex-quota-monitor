...
The whole panel is a grab surface: drag it from the title row, the quota readout, or any gap between rows. Only controls that own the pointer keep it -- buttons, the unit selector, the disclosure summaries, and the invisible corner resize targets. A pointerdown on the scrollbar gutter still belongs to the scroll container, so scrolling never turns into a drag. Hover and drag any of the four corners to resize the panel and its typography; the bottom-right target also supports arrow-key resizing when focused. Compact and expanded positions/sizes are remembered separately. Temporary viewport clamping does not overwrite the saved anchor. A 64px top safe area avoids the app title bar, and the overlay is explicitly marked as a non-native-drag region. The title row is pinned to the top of the panel, so the quota readout and the window controls stay reachable while the detail list is scrolled.
...
The bottom of the settings panel names the installed build and checks the public repository `plugin.json` at most once per day. A newer cachebuster produces an update notice and link. The button on the right forces a fresh check and briefly reads `Up to date` when the installed build is current. The request sends no local version, account data, or conversation content; failures are silent and do not affect offline monitoring.
