        ...
        try {
          ...
          await context.route('**/*', route => {
            ...
            return route.fulfill({ contentType: 'text/html; charset=utf-8', body: `<!doctype html><html lang="zh-CN"><meta charset="utf-8">
              ...
              <button data-app-action-sidebar-thread-row data-app-action-sidebar-thread-id="one" data-app-action-sidebar-thread-active="true">任务一</button>
              <button data-app-action-sidebar-thread-row data-app-action-sidebar-thread-id="two" data-app-action-sidebar-thread-active="false">任务二</button>
              <button data-app-action-sidebar-thread-row data-app-action-sidebar-thread-id="missing" data-app-action-sidebar-thread-active="false">无记录任务</button>
              </html>` });
          });
          ...
          const push = key => page.evaluate(payload => window.__codexContextTokenInspectorUpdate(payload), fixture.payloads[key]);
