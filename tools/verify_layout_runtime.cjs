...
const context = vm.createContext({window:{innerWidth:1000,innerHeight:900},document:{getElementById:()=>mascot},
  ...
  dockVerticalY:(y,h,p)=>Math.max(64,Math.min(h-p-8,y)),initialPanelLayout:()=>({}),saved:0,saveLayout:()=>context.saved++,
...
root.__ctiLayout.expanded={edge:'left',y:100,width:300}; context.applyStoredHudPosition(root);
...
context.revealDock(root,true); assert.equal(root.dataset.revealed,'true'); assert.equal(style.left,'52px');
