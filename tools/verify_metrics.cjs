...
assert.equal(context.labeled('sample','1'), 'Sample: 1'); assert.equal(context.parenthesized('x'), '(x)');
assert.equal(context.joined(['a','b']), 'a, b'); context.language='zh';
assert.equal(context.labeled('sample','1'), '示例：1'); assert.equal(context.parenthesized('x'), '（x）');
assert.equal(context.joined(['a','b']), 'a，b');
