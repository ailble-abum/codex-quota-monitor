...
(async () => {
  ...
  for (const engine of [chromium, webkit]) {
    ...
    try {
      ...
      const quota = remaining => ({status:'live', accountKey:'synthetic-account', updatedAt:Date.now()/1000,
        windows:[{key:'five-hour', remaining, duration:300, resetsAt:resetAt}]});
      payload.quota = quota(80);
      await publish();
      payload.quota = quota(20);
      await publish();
      assert.ok((await page.locator('#cti-context-hint').textContent()).includes('额度剩余不多'));
      payload.quota = quota(10);
      await publish();
      assert.ok((await page.locator('#cti-context-hint').textContent()).includes('低于或等于 10%'));
      payload.quota = quota(0);
      payload.quota.ordinaryUsageAllowed = false;
      await publish();
      assert.ok((await page.locator('#cti-context-hint').textContent()).includes('已达上限'));
      payload.quota = quota(80);
      await publish();
      assert.ok((await page.locator('#cti-context-hint').textContent()).includes('额度恢复'));
      payload.summaries[0].latest_context_percent = 85;
      await publish();
      assert.ok((await page.locator('#cti-context-hint').textContent()).includes('上下文偏高'));
      payload.summaries[0].latest_context_percent = 95;
      await publish();
      assert.ok((await page.locator('#cti-context-hint').textContent()).includes('接近窗口上限'));
      payload.healthThreadId = 'one';
      payload.health = {count:1, latestAt:'synthetic-compaction-1'};
      await publish();
      payload.health = {count:2, latestAt:'synthetic-compaction-2'};
      await publish();
      assert.ok((await page.locator('#cti-context-hint').textContent()).includes('上下文压缩'));
      ...
      await publish();
      payload.history = {samples: 2, spanSeconds: 86400, models: ['safe-model'], weekly: {
      ...
      for (const [reaction, animation] of [['hello','cti-mascot-greet'], ['notice','cti-mascot-notice']]) {
        assert.equal(await page.locator('#codex-context-token-inspector-mascot').evaluate((node, reaction) => {
          node.dataset.reaction = reaction;
          return getComputedStyle(node.querySelector('img')).animationName;
        }, reaction), animation);
      }
      await page.emulateMedia({reducedMotion:'reduce'});
      assert.equal(await page.locator('#codex-context-token-inspector-mascot img').evaluate(node => getComputedStyle(node).animationName), 'none');
      await page.locator('#codex-context-token-inspector-mascot').evaluate(node => delete node.dataset.reaction);
      ...
      await page.emulateMedia({reducedMotion: 'no-preference'});
      const headX = novaBox.x + novaBox.width / 2;
      const headY = novaBox.y + 8;
      await page.mouse.move(headX, headY);
      assert.equal(await nova.getAttribute('data-expression'), 'happy');
      await page.mouse.down();
      await page.waitForTimeout(380);
      assert.equal(await nova.getAttribute('data-expression'), 'pet');
      await page.mouse.move(headX + 18, headY + 18);
      assert.equal(await nova.getAttribute('data-expression'), 'pet', 'pet reaction must remain active during movement');
      await page.mouse.up();
      assert.notEqual(await page.locator('.cti-hud').getAttribute('data-dock-pinned'), 'true', 'pet release must suppress click');
      await page.waitForTimeout(1450);
      const beforeDrag = await nova.boundingBox();
      await page.mouse.move(headX, headY + 20);
      await page.mouse.down();
      await page.mouse.move(headX, headY - 45, {steps: 5});
      await page.mouse.up();
      assert.notEqual(await page.locator('.cti-hud').getAttribute('data-dock-pinned'), 'true', 'drag release must suppress click');
      assert.ok((await nova.boundingBox()).y < beforeDrag.y - 4, 'drag must move mascot within viewport bounds');
      assert.equal(await nova.getAttribute('data-reaction'), 'land');
      assert.equal(await nova.locator('img').evaluate(node => getComputedStyle(node).animationName), 'cti-mascot-land');
      await page.emulateMedia({reducedMotion: 'reduce'});
      assert.equal(await nova.locator('img').evaluate(node => getComputedStyle(node).animationName), 'none');
      await page.waitForTimeout(950);
      await nova.click();
      assert.equal(await page.locator('.cti-hud').getAttribute('data-dock-pinned'), 'true');
      assert.equal(await nova.getAttribute('aria-pressed'), 'true');
      await nova.click();
      assert.equal(await page.locator('.cti-hud').getAttribute('data-dock-pinned'), 'false');
      await nova.focus();
      assert.equal(await page.locator('.cti-hud').getAttribute('data-revealed'), 'true');
      await page.evaluate(() => localStorage.setItem('cti-companion-motion', 'false'));
      await publish();
      assert.equal(await nova.getAttribute('data-motion'), 'false');
      await page.waitForTimeout(950);
      await page.mouse.move(200, 20);
      await page.mouse.move((await nova.boundingBox()).x + 20, (await nova.boundingBox()).y + 8);
      assert.equal(await nova.getAttribute('data-reaction'), null, 'motion off must suppress pointer reaction');
      await page.evaluate(() => localStorage.setItem('cti-companion-motion', 'true'));
      await publish();
      await page.mouse.down();
      ...
      if (!await page.locator('[data-settings]').evaluate(node => node.hidden))
        await page.locator('[data-settings-toggle]').click();
      await page.locator('[data-cti-toggle]').click();
      assert.equal(await page.locator('.cti-hud').getAttribute('data-collapsed'), 'true');
      await publish();
      assert.equal(await page.locator('.cti-hud').getAttribute('data-collapsed'), 'false');
      assert.equal(await page.locator('[data-settings]').evaluate(node => node.hidden), false);
      assert.equal(await page.locator('[data-update] [role="alertdialog"]').count(), 1);
      await page.waitForFunction(() => {
        const root = document.querySelector('.cti-hud').getBoundingClientRect();
        const prompt = document.querySelector('[data-update] [role="alertdialog"]').getBoundingClientRect();
        return prompt.top >= root.top && prompt.bottom <= root.bottom;
      });
      if (process.argv[3]) await page.locator('.cti-hud').screenshot({path:path.join(process.argv[3],`${engine.name()}-auto-update.png`)});
      await page.locator('[data-update-dismiss]').click();
      assert.equal(await page.evaluate(() => localStorage.getItem('cti-update-dismissed-version')), '2.0.0');
      await page.evaluate(() => { document.querySelector('.cti-hud').__ctiAutoUpdate = null; });
      await publish();
      assert.equal(await page.locator('[data-update] [role="alertdialog"]').count(), 0);
      payload.update.latestSemver = '2.0.1';
      await publish();
      assert.ok((await page.locator('[data-update] [role="alertdialog"]').textContent()).includes('2.0.1'));
      await page.locator('[data-update-dismiss]').click();
